...
import type { BlogMeta } from '../data/blogMeta';

const CATEGORIES = ['All', 'Comparison', 'ESP', 'Aimbot', 'Spoofing', 'Anti-Cheat', 'Guide', 'Technical', 'Safety'];

function BlogCard({ post, featured = false }: { post: BlogMeta; featured?: boolean }) {
  return (
    <Link
      to={`/blog/${post.slug}`}
...
export function BlogListPage() {
  ...
  const filtered = activeCategory === 'All'
    ? BLOG_META
    : BLOG_META.filter(p => p.category === activeCategory);
...
export function BlogPostPage() {
  ...
  const postMeta = BLOG_META.find(p => p.slug === slug);
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    import('../data/blogBodies').then((m) => {
      if (!cancelled) setBody(m.BLOG_BODIES[slug] ?? '');
    });
    return () => {
      cancelled = true;
    };
  ...
  if (!postMeta) {
    return (
      <div style={{ minHeight: '80vh', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 20 }}>
        <p style={{ color: 'var(--text-muted)', fontFamily: 'var(--font-body)', fontSize: '1.1rem' }}>Post not found.</p>
        <Link to="/blog" className="btn-ghost" style={{ fontSize: '0.875rem' }}>← Back to Blog</Link>
      </div>
    );
  }

  const post = { ...postMeta, body };
  const related = BLOG_META.filter(p => p.slug !== slug && p.category === post.category).slice(0, 2);
    ...
    <div style={{ background: 'var(--bg-void)', minHeight: '100vh' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
          ...
          <span style={{ color: 'var(--border-bright)', fontSize: '0.5rem' }}>●</span>
        ...
        <div style={{
          ...
          <div>
            ...
            <p style={{ fontFamily: 'var(--font-body)', fontSize: '0.875rem', color: 'var(--text-secondary)' }}>
              Full ESP — enemies, chests, portals, traps — $35/month. Wipe-updated loader on Zadeyo.
            </p>
          </div>
